...
*/
/*
This also saves data to text file 
*/
#include <stdio.h>
#include <string.h>
// #include <wmmintrin.h>
...
void insertPlayers()
{
    ...
    scanf("%d", &noOfPlayers);
    int i = totalPlayersAdded;
    int temp = totalPlayersAdded;
    for (; i < (temp + noOfPlayers); i++)
    {
...
void addPlayersToFile()
{
    ...
    for (int i = 0; i < totalPlayersAdded; i++)
    {
        fprintf(file, "%s %d %d %d\n", players[i].name, players[i].totalMatches, players[i].wickets, players[i].runs);
    }
    fclose(file);
}
...
void readPlayersFromFile()
{
    ...
    if (file != NULL)
    {
        totalPlayersAdded = fscanf(file, "%s %d %d %d", players[0].name, &players[0].totalMatches, &players[0].wickets, &players[0].runs);
        totalPlayersAdded -= 1;
        printf("\n%d\n", totalPlayersAdded);
        for (int i = 1; i < totalPlayersAdded; i++)
        {
            fscanf(file, "%s %d %d %d", players[i].name, &players[i].totalMatches, &players[i].wickets, &players[i].runs);
        }
        fclose(file);
    }
}